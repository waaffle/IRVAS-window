    ...
    elem.forEach((item, i) => {
      item.addEventListener(event, () => {
        if (item.getAttribute('type') == 'checkbox') {
          elem.forEach((box, j) => {
            if (i == j) box.checked = true;else box.checked = false;
          });
        ...
        if (state['width'] && state['height']) {
          (0,_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(true);
        }
        if (state['profile']) {
          (0,_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(true, true);
        }
      });
    });
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.container1', '2025-11-23');
